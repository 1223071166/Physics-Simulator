...
export default function App() {
  ...
  const [electricFields, setElectricFields] = useState([{ id: 1, visible: true, start: [-4, 0, -4], end: [0, 4, 0], rotation: [0, 0, 0], magnitude: 20 }]);
  const [magneticFields, setMagneticFields] = useState([{ id: 2, visible: true, start: [1, 0, 1], end: [5, 4, 5], rotation: [45, 0, 0], magnitude: 20 }]);
  ...
  const addEField = () => { setElectricFields([...electricFields, { id: Date.now(), visible: true, start: [-2, -2, -2], end: [2, 2, 2], rotation: [0, 0, 0], magnitude: 20 }]); setMenu({ ...menu, visible: false }); };
  const addBField = () => { setMagneticFields([...magneticFields, { id: Date.now(), visible: true, start: [-2, -2, -2], end: [2, 2, 2], rotation: [0, 0, 0], magnitude: 20 }]); setMenu({ ...menu, visible: false }); };
    ...
    <div onClick={() => {setMenu({ ...menu, visible: false });setMenu2({ ...menu2, visible: false })}} style={{ display: 'flex', width: '100vw', height: '100vh', margin: 0, backgroundColor: '#111', color: 'white', fontFamily: 'sans-serif' }}>
      ...
      >
        <Canvas camera={{ position: [10, 10, 10], fov: 50,up: [0, 0, 1] }}>
          ...
          <OrbitControls ref={controlsRef} makeDefault enableDamping dampingFactor={0.05} listenToKeyEvents={window} panSpeed={2} keyPanSpeed={20} />
        </Canvas>
      </div>
      ...
      >
        <h2 style={{ marginTop: 0, borderBottom: '1px solid #555', paddingBottom: '10px' }}>Physics Inspector</h2>
         ...
         <div style={{ display: 'flex', gap: '8px', marginBottom: '15px' }}>
          <button onClick={() => setIsRunning(true)} style={{ flex: 1, padding: '8px', backgroundColor: isRunning ? '#27ae60' : '#2ecc71', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>
            ▶ Run
          </button>
        ...
        {particles.map((p, i) => <ParticleCard key={p.id} particle={p} index={i} onUpdate={(id, prop, idx, val) => updateEntity(setParticles, id, prop, idx, val)} onDelete={removeParticle} />)}
